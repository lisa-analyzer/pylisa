...
public class FunctionApply extends NaryExpression {
    ...
    public FunctionApply(CFG cfg, CodeLocation location, Expression identifier, Expression[] params) {
    ...
    public String toString() {
        return getSubExpressions()[0] + "(" + Arrays.stream(getSubExpressions()).toList().subList(1, getSubExpressions().length) + ")";
    }

    private static Expression[] prependReceiver(Expression[] params, Expression receiver) {
    ...
    public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> forwardSemanticsAux(InterproceduralAnalysis<A, D> interprocedural, AnalysisState<A> state, ExpressionSet[] params, StatementStore<A> expressions) throws SemanticException {
        ...
        for (SymbolicExpression identifier : params[0]) {
            ...
            if (identifier instanceof LazyEvaluatedVariadicExpression lazyEval) {
                ...
                if (e instanceof VariadicExpression ve) {
                    ...
                    ve.getVarargsIndex().forEach((name, index) -> {
                        builder.varargsOperand(name, oldOperands[index]);
                    });
                    int j = 1;
                    for (Expression le : lazyEval.additionalExpressions) {
                        builder.varargsOperand(le.toString(), params[j].elements.stream().findFirst().get()); // todo: fix that!
                    }
                    ...
                    for (Type t : runtimeTypes) {
                        if (t instanceof PyClassType pct) {
                            ClassInstantiation ci = new ClassInstantiation(this.getCFG(), getLocation(), pct, getSubExpressions());
                        ...
                        if (t instanceof PyFunctionType pft) {
                            ...
                            if (cm instanceof NativeCFG cfg) {
                                c = new NativeCall(this.getCFG(), getLocation(), Call.CallType.STATIC, "", "$call", List.of(cfg), Arrays.copyOfRange(getSubExpressions(), 1, getSubExpressions().length));
                            } else if (cm instanceof CFG cfg) {
                                c = new CFGCall(this.getCFG(), getLocation(), Call.CallType.STATIC, "", "$call", List.of(cfg), Arrays.copyOfRange(getSubExpressions(), 1, getSubExpressions().length));
                            }
                            if (c != null) {
                                result = result.lub(c.forwardSemantics(state, interprocedural, expressions));
                            }
                        }
                        //AttributeAccess access = new AttributeAccess(this.getCFG(), SyntheticLocation.INSTANCE, getSubExpressions()[0],  "__new__");

                        //FunctionApply apply = new FunctionApply(getCFG(), SyntheticLocation.INSTANCE, access, new Expression[]{});
                        //result = result.lub(apply.forwardSemantics(state, interprocedural, expressions));
                    }
                    //result = result.lub(analysis.smallStepSemantics(state, identifier, this));
                }
            }
        ...
        /*if (right instanceof Constant) {
            ...
            if (left instanceof LazyEvaluatedVariadicExpression lazyEval) {
                SymbolicExpression e = lazyEval.getExpression();
                if (e instanceof VariadicExpression ve) {
                    VariadicExpression.Builder builder = new VariadicExpression.Builder()
                            .operator(ve.getOperator())
                            ...
                            .location(ve.getCodeLocation());
                    SymbolicExpression[] oldOperands = ve.getOperands();
                    /*for (SymbolicExpression op : oldOperands) {
                        builder.operand(op);
                    }
                    ve.getVarargsIndex().forEach((name, index) -> {
                        builder.varargsOperand(name, oldOperands[index]);
                    ...
                    for (Expression le : lazyEval.additionalExpressions) {
                        builder.varargsOperand(le.toString(), right);
                    }
                    VariadicExpression expr = builder.build();
                    return interprocedural.getAnalysis().smallStepSemantics(state, expr, this);
                }
