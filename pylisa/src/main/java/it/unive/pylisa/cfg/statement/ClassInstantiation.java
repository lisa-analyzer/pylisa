			...
			for (Type t : runtimeTypes) {
				if (t instanceof PyClassType pct) {
					GlobalVariable variable = new GlobalVariable(Untyped.INSTANCE,
							"$" + pct.getUnit().getName() + "::__new__", getLocation());
					...
					for (Type _t : types) {
						if (_t instanceof PyFunctionType ft) {
							...
							if (c != null) {
								InstrumentedReceiverRef syntheticVariable = new InstrumentedReceiverRef(this.getCFG(),
										getLocation(), false);
								...
								for (SymbolicExpression e : result.getExecutionExpressions()) {
									Set<Type> types_result = interprocedural.getAnalysis().getRuntimeTypesOf(result, e,
											this);
									for (Type __t : types_result) {
										if (__t instanceof ReferenceType rt) {
											PythonUnitAttributeAccessRef unitAttributeAccessRef = new PythonUnitAttributeAccessRef(
													this.getCFG(), getLocation(), pct.getUnit(),
													new Global(getLocation(), pct.getUnit(), "__init__", false));
											if (e instanceof Identifier) {
												...
												result = result
														.lub(a.forwardSemantics(result, interprocedural, expressions));
											}
										}
									}
								}
