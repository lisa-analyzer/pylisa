...
public class PyFunctionType implements UnitType {
    ...
    public static PyFunctionType register(String name, FunctionUnit unit) {
        return types.computeIfAbsent(name, x -> new PyFunctionType(name, unit));
    }

    public static PyFunctionType lookup(String name) {
    ...
    private PyFunctionType(String name, FunctionUnit unit) {
        this.name = name;
        this.unit = unit;
    }
