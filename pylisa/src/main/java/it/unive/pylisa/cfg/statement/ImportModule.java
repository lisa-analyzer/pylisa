...
public class ImportModule extends Expression {
     ...
     *                 program
     */
    protected ImportModule(CFG cfg, CodeLocation location) {
        super(cfg, location);
    }

    public ImportModule(CFG cfg, CodeLocation location, String moduleName, CompilationUnit pythonModuleUnit) {
        this(cfg, location);
    ...
    public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> forwardSemantics(AnalysisState<A> entryState, InterproceduralAnalysis<A, D> interprocedural, StatementStore<A> expressions) throws SemanticException {
        ...
        PyAssign assign = new PyAssign(this.getCFG(), getLocation(), v, new ModuleLiteral(this.getCFG(), getLocation(), pythonModuleUnit));
        state = assign.forwardSemantics(state, interprocedural, expressions);
        return state;
        //return state;
        //if (state.getExecutionInfo(MetaSysModules.INFO_KEY) == null)
        //    state = state.storeExecutionInfo(MetaSysModules.INFO_KEY, new MetaSysModules());
        //state = state.storeExecutionInfo(MetaSysModules.INFO_KEY, state.getExecutionInfo(MetaSysModules.INFO_KEY, MetaSysModules.class)
        //        .putModule(moduleName, new PythonModuleLattice(false, pythonModuleUnit)));

        //Type type = PyModuleType.lookup(pythonModuleUnit.getName());

        //PyNewObj pyNewObj = new PyNewObj(this.getCFG(), getLocation(), "$moduleInit", type);
        //return pyNewObj.forwardSemantics(state, interprocedural, expressions);
    }
