...
/**
 * Python's bitwise or ({@code a | b}). It evaluates {@code a} and {@code b},
		...
		for (Type tl : rtsl) {
			for (Type tr : rtsr) {
				if (tr.canBeAssignedTo(tl)) {
					// int | int (and subtypes thereof): call int.__or__,
					// falling back to int.__ror__ if it does not resolve
					UnresolvedCall or = new UnresolvedCall(
							getCFG(),
							getLocation(),
							CallType.STATIC,
							null,
					...
					if (orResolves)
						result = result.lub(or.forwardSemantics(state, interprocedural, expressions));
					else {
						UnresolvedCall ror = new UnresolvedCall(
								getCFG(),
								getLocation(),
								CallType.STATIC,
								null,
								...
								getRight(),
								getLeft());
						result = result.lub(ror.forwardSemantics(state, interprocedural, expressions));
					}
				}
			}
		}
