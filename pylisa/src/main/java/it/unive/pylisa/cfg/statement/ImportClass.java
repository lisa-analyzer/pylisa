...
public class ImportClass extends Expression {
     ...
     *                 program
     */
    protected ImportClass(CFG cfg, CodeLocation location) {
        super(cfg, location);
    }

    public ImportClass(CFG cfg, CodeLocation location, String className, ClassUnit classUnit) {
        this(cfg, location);
    ...
    public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> forwardSemantics(AnalysisState<A> entryState, InterproceduralAnalysis<A, D> interprocedural, StatementStore<A> expressions) throws SemanticException {
        ...
        PyAssign assign = new PyAssign(this.getCFG(), getLocation(), v, new ClassLiteral(this.getCFG(), getLocation(), classUnit));
