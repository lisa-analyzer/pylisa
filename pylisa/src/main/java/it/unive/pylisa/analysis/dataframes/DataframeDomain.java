...
import it.unive.pylisa.analysis.dataframes.transformation.operations.BaseOperation;
...
public class DataframeDomain extends
		NonRelationalValueCartesianProduct<DataframeDomain, DataframeGraphDomain, ConstantPropagation>
		implements DataframeAwareDomain<DataframeDomain, DataframeGraphDomain> {

	public DataframeDomain() {
		this(new DataframeGraphDomain().top(), new ConstantPropagation().top());
	}

	public DataframeDomain(DataframeGraphDomain left, ConstantPropagation right) {
		super(left, right);
	}

	@Override
	protected DataframeDomain mk(DataframeGraphDomain left, ConstantPropagation right) {
	...
	public DataframeDomain eval(ValueExpression expression, ValueEnvironment<DataframeDomain> environment,
		...
		ValueEnvironment<DataframeGraphDomain> lenv = new ValueEnvironment<>(left);
	...
	private DataframeDomain reducedUnary(ProgramPoint pp, ValueEnvironment<DataframeGraphDomain> lenv,
		...
		if (unary.getOperator() == ReadDataframe.INSTANCE) {
			...
			DataframeGraphDomain df = new DataframeGraphDomain(
					new BaseOperation("file", filename.getConstantAs(String.class)));
			return new DataframeDomain(df, right.bottom());
		} else if (unary.getOperator() == Statistics.INSTANCE) {
			DataframeGraphDomain df = extractDataFrame(unary.getExpression(), lenv, pp);
			DataframeGraphDomain stat = new DataframeGraphDomain(df,
					new BaseOperation("stats"));
			return new DataframeDomain(stat, right.bottom());
		} else if (unary.getOperator() == StructuralInfo.INSTANCE) {
			DataframeGraphDomain df = extractDataFrame(unary.getExpression(), lenv, pp);
			DataframeGraphDomain info = new DataframeGraphDomain(df,
					new BaseOperation("info"));
			...
			DataframeGraphDomain conv = new DataframeGraphDomain(df,
					new BaseOperation("conv", ((TypeConversion) unary.getOperator()).getType()));
			return new DataframeDomain(conv, right.bottom());
		} else
			return bottom();
	}

	private DataframeDomain reducedBinary(ProgramPoint pp, ValueEnvironment<DataframeGraphDomain> lenv,
		...
		if (binary.getOperator() == ColumnAccess.INSTANCE) {
			DataframeGraphDomain df = extractDataFrame(binary.getLeft(), lenv, pp);
			...
			DataframeGraphDomain ca = new DataframeGraphDomain(df,
					new BaseOperation("col_access", col.getConstantAs(String.class)));
	...
	private DataframeDomain reducedTernary(ProgramPoint pp, ValueEnvironment<DataframeGraphDomain> lenv,
		...
		if (ternary.getOperator() == ProjectRows.INSTANCE) {
			DataframeGraphDomain df = extractDataFrame(ternary.getLeft(), lenv, pp);
			...
			DataframeGraphDomain pr = new DataframeGraphDomain(df,
					new BaseOperation("project_rows", start.getConstantAs(Integer.class),
			...
			DataframeGraphDomain pr = new DataframeGraphDomain(df,
					new BaseOperation("set_opt", key.getConstantAs(String.class),
							value.getConstant()));
	...
	private DataframeGraphDomain extractDataFrame(SymbolicExpression expr,
			ValueEnvironment<DataframeGraphDomain> env, ProgramPoint pp) throws SemanticException {
	...
	@Override
	public DataframeGraphDomain getDataFrame() {
		return left;
	}

	@Override
	public boolean sameDataFrame(DataframeGraphDomain other) {
		return left.equals(other);
	}

	@Override
	public DataframeDomain createDataframe(DataframeGraphDomain value) {
