...
/**
 * The domain evaluating expressions over {@link DataframeGraphDomain}
...
public class DataframeGraphValueDomain implements ValueDomain<DataframeGraphDomain> {
	...
	@Override
	public DataframeGraphDomain makeLattice() {
	...
	public DataframeGraphDomain assign(
			DataframeGraphDomain state,
			Identifier id,
			ValueExpression expression,
			ProgramPoint pp,
			SemanticOracle oracle)
			throws SemanticException {
		DataframeGraphDomain sss = smallStepSemantics(state, expression, pp, oracle);
	...
	public DataframeGraphDomain smallStepSemantics(
		...
		if (expression instanceof Identifier)
			return visit(state, (Identifier) expression, pp);
		else if (expression instanceof Constant)
			return visit(state, (Constant) expression, pp, oracle);
		else if (expression instanceof PushAny)
			return visit(state, (PushAny) expression, pp, oracle);
		else if (expression instanceof UnaryExpression) {
			UnaryExpression unary = (UnaryExpression) expression;
			DataframeGraphDomain arg = smallStepSemantics(state, (ValueExpression) unary.getExpression(), pp, oracle);
			return visit(unary, arg, pp, oracle);
		} else if (expression instanceof BinaryExpression) {
			BinaryExpression binary = (BinaryExpression) expression;
			DataframeGraphDomain left = smallStepSemantics(state, (ValueExpression) binary.getLeft(), pp, oracle);
			DataframeGraphDomain right = smallStepSemantics(left, (ValueExpression) binary.getRight(), pp, oracle);
			return visit(binary, left, right, pp, oracle);
		} else if (expression instanceof TernaryExpression) {
			TernaryExpression ternary = (TernaryExpression) expression;
			DataframeGraphDomain left = smallStepSemantics(state, (ValueExpression) ternary.getLeft(), pp, oracle);
			DataframeGraphDomain middle = smallStepSemantics(left, (ValueExpression) ternary.getMiddle(), pp, oracle);
			DataframeGraphDomain right = smallStepSemantics(middle, (ValueExpression) ternary.getRight(), pp, oracle);
			return visit(ternary, left, middle, right, pp, oracle);
		}
		return state;
	}
	...
	public DataframeGraphDomain assume(
			...
			throws SemanticException {
		return new DataframeGraphDomain(
				CP_DOMAIN.assume(state.constants, expression, src, dest, oracle),
				state.graph,
				state.pointers,
				state.operations);
	}
	...
	public Satisfiability satisfies(
			...
			SemanticOracle oracle)
			throws SemanticException {
		Satisfiability c = CP_DOMAIN.satisfies(state.constants, expression, pp, oracle);
	...
	public static DataframeGraphDomain visit(
		...
		if (!arg.constStack.isBottom()
				&& CP_DOMAIN.canProcess(expression, pp, oracle))
	...
	private static DataframeGraphDomain delegateToConstants(
		...
		return new DataframeGraphDomain(
				arg.constants,
				CP_DOMAIN.eval(arg.constants, expression, pp, oracle),
				arg.graph,
	...
	private static DataframeGraphDomain doListAppend(
		...
		return new DataframeGraphDomain(
				right.constants,
				CP_DOMAIN.eval(right.constants, listconst, pp, oracle),
				right.graph,
	...
	private static DataframeGraphDomain doSliceCreation(
		...
		return new DataframeGraphDomain(
				right.constants,
				CP_DOMAIN.eval(right.constants, slice, pp, oracle),
				right.graph,
				right.pointers.setStack(NO_IDS),
				right.operations);
	}
	...
	private static DataframeGraphDomain doDictPut(
		...
		return new DataframeGraphDomain(
				right.constants,
				CP_DOMAIN.eval(right.constants, newdict, pp, oracle),
				right.graph,
				right.pointers.setStack(NO_IDS),
				right.operations);
	}
	...
	private static String getCaller() {
		...
		// 2: DataframeGraphValueDomain.getCaller()
		// 3: DataframeGraphValueDomain.cleanStack()
		// 4: caller
	...
	public static DataframeGraphDomain visit(
		...
		if (isDataframeRelated(expression, pp, oracle))
			return new DataframeGraphDomain(
					state.constants,
					state.graph,
					state.pointers.setStack(NO_IDS.top()),
					state.operations);
		else
			return new DataframeGraphDomain(
					state.constants,
					state.constStack.top(),
					state.graph,
					state.pointers.setStack(NO_IDS),
					state.operations);
	}
	...
	public static DataframeGraphDomain visit(
		...
		return new DataframeGraphDomain(
				state.constants,
				CP_DOMAIN.eval(state.constants, expression, pp, oracle),
				state.graph,
				state.pointers.setStack(NO_IDS),
				state.operations);
	}
	...
	public static DataframeGraphDomain visit(
		...
		if (expression instanceof AllocationSite)
			expression = DataframeGraphDomain.stripFields((AllocationSite) expression);

		if (state.constants.getKeys().contains(expression))
			return new DataframeGraphDomain(
					state.constants,
					state.constants.getState(expression),
					state.graph,
					state.pointers.setStack(NO_IDS),
					state.operations);
		else if (state.pointers.getKeys().contains(expression))
			return new DataframeGraphDomain(
					state.constants,
					state.graph,
					state.pointers.setStack(state.pointers.getState(expression)),
					state.operations);
		else
			return new DataframeGraphDomain(
					state.constants,
