...
public class Patch extends VariadicExpression implements PluggableStatement {
	...
	public Patch(
			...
			Expression[] params) {
		super(cfg, location, "fastapi.patch", params, new HashMap<>() {
			{
				put("path", 1);
	...
	public static Patch build(
			CFG cfg,
			CodeLocation location,
			Expression[] exprs) {
		return new Patch(cfg, location, exprs);
	}
