...
public class CreatePublisher extends NaryExpression implements PluggableStatement {
    ...
    public <A extends AbstractState<A, H, V, T>, H extends HeapDomain<H>, V extends ValueDomain<V>, T extends TypeDomain<T>> AnalysisState<A, H, V, T> expressionSemantics(InterproceduralAnalysis<A, H, V, T> interprocedural, AnalysisState<A, H, V, T> state, ExpressionSet<SymbolicExpression>[] params, StatementStore<A, H, V, T> expressions) throws SemanticException {
        ...
        /*
            0. Create an empty Set of SymbolicExpressions
            1. Get computed expressions from the new analysis state
            2. For every computed expressions:
                a Rewrite the HeapState (ExpressionSet of ValueExpression
                b. Then, for every SymbolicExpression of params[2]
                    i. add a new BinaryExpression (with StringFormat operator) to the set.
        ...
        AnalysisState<A,H,V,T> newPublisherAS = publisherObj.expressionSemantics(interprocedural, aigSemanticsNodeName, Arrays.copyOfRange(params, 1, params.length), expressions);
