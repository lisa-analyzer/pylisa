...
/**
 * Pluggable statement for {@code s3_client.delete_object(Bucket=..., Key=...)}.
 */
public class S3DeleteObject extends VariadicExpression implements PluggableStatement {
	...
	public S3DeleteObject(
			...
			Expression[] params) {
		super(cfg, location, "s3.delete_object", params, new HashMap<>() {
			{
				put("Bucket", 1);
				put("Key", 2);
			}
	...
	protected int compareSameClassAndParams(
			Statement o) {
		S3DeleteObject other = (S3DeleteObject) o;
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdVariadicSemantics(
		...
		it.unive.lisa.symbolic.value.VariadicExpression expr = new it.unive.lisa.symbolic.value.VariadicExpression.Builder()
				.operator(S3ResourceCreation.DELETE_OBJECT)
	...
	public static S3DeleteObject build(
			CFG cfg,
			CodeLocation location,
			Expression[] exprs) {
		return new S3DeleteObject(cfg, location, exprs);
	}
