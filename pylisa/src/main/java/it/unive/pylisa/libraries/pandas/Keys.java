...
public class Keys extends it.unive.lisa.program.cfg.statement.UnaryExpression implements PluggableStatement {
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdUnarySemantics(
		...
		/*
		 * CodeLocation loc = getLocation(); PyClassType dftype =
		 * PyClassType.lookup(LibrarySpecificationProvider.PANDAS_DF);
		 * PyClassType seriestype =
		 ...
		 * AnalysisState<A> copied = PandasSemantics.copyDataframe(state, deref,
		 * st); AnalysisState<A> result = state.bottom(); for
		 * (SymbolicExpression id : copied.getComputedExpressions()) {
		 * UnaryExpression transform = new UnaryExpression(seriestype, id, new
		 * AccessKeys(0), loc); AnalysisState<A> tmp =
		 * copied.smallStepSemantics(transform, st); HeapReference ref = new
		 * HeapReference(seriesref, id, loc); result =
		 * result.lub(tmp.smallStepSemantics(ref, st)); } return result;
		 */
		return state;
	}
}
