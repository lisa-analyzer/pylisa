...
/**
 * Pluggable statement for {@code s3_client.get_object(Bucket=..., Key=...)}.
 */
public class S3GetObject extends VariadicExpression implements PluggableStatement {
	...
	public S3GetObject(
			...
			Expression[] params) {
		super(cfg, location, "s3.get_object", params, new HashMap<>() {
			{
				put("Bucket", 1);
				put("Key", 2);
			}
	...
	protected int compareSameClassAndParams(
			Statement o) {
		S3GetObject other = (S3GetObject) o;
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdVariadicSemantics(
		...
		it.unive.lisa.symbolic.value.VariadicExpression expr = new it.unive.lisa.symbolic.value.VariadicExpression.Builder()
				.operator(S3ResourceCreation.GET_OBJECT)
	...
	public static S3GetObject build(
			CFG cfg,
			CodeLocation location,
			Expression[] exprs) {
		return new S3GetObject(cfg, location, exprs);
	}
