...
public class APIRouterIncludeRouter extends VariadicExpression implements PluggableStatement {

	protected Statement st;

	public APIRouterIncludeRouter(
			CFG cfg,
		...
		super(cfg, location, "include_router", params, new HashMap<>() {
			{
				...
				put("tags", 3);
				put("dependencies", 4);
				put("default_response_class", 5);
				put("responses", 6);
				put("callbacks", 7);
				put("deprecated", 8);
				put("include_in_schema", 9);
	...
	protected int compareSameClassAndParams(
			Statement o) {
		APIRouterIncludeRouter other = (APIRouterIncludeRouter) o;
	...
	public static APIRouterIncludeRouter build(
			CFG cfg,
			CodeLocation location,
			Expression[] exprs) {
		return new APIRouterIncludeRouter(cfg, location, exprs);
	}
