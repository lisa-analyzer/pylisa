...
public class EndpointChecker {
	...
	public void doCheck(
		...
		for (Endpoint endpoint : endpoints) {
			...
			if (endpoint.getMethod() == Method.GET)
				validateGET(tool, unit, endpoint);
			if (endpoint.getMethod() == Method.POST)
				validatePOST(tool, unit, endpoint);
			if (endpoint.getMethod() == Method.PUT)
				validatePUT(tool, unit, endpoint);
	...
	public void validateGET(
		...
		if (pathVariableName != null && methodVariableName != null && !pathVariableName.equals(methodVariableName)) {
			tool.warnOn(unit, String.format(
					"Path variable name of endpoint '%s' differs from one specified in method. | Path variable name: %s | Method variable name: %s",
					endpoint.getFullPath(), pathVariableName, methodVariableName));
		...
		if (!"string".equals(methodVariableType) && !"numeric".equals(methodVariableType)) {
			tool.warnOn(unit, String.format(
					"This GET endpoint's path method accepts non-string or non-numeric parameter: %s | Variable's type: %s",
					endpoint.getFullPath(), methodVariableType));
	...
	public void validatePOST(
		...
		if (!endpoint.getMethodPathVariable().isEmpty()) {
			...
			if (!oneOfMethodVarsIsCustomDef) {
				tool.warnOn(unit, "This POST endpoint " + endpoint.getFullPath()
						+ " does not accept any argument of custom definition, t.i. model. Only primitives.");
				endpoint.addIssue(
						"Endpoint does not accept any argument of custom definition, t.i. model. Only primitives.");
			}
		}
	}
	...
	public void validatePUT(
		...
		if (!idForUpdateSpecified && objectProvided) {
			tool.warnOn(unit, "This PUT endpoint " + endpoint.getFullPath()
					+ " does not accept any arguments to specify the object to update.");
			...
			tool.warnOn(unit, "This PUT endpoint " + endpoint.getFullPath()
					+ " does not accept any arguments as updates target ID.");
	...
	public Boolean validateDELETE(
		...
		if (delete != null) {
			...
			PyStringLiteral pathLiteral = (PyStringLiteral) Arrays.stream(pathWrapper.getSubExpressions()).toList()
					.get(1);
			String pathVariable = Endpoint.extractPathVariable(pathLiteral.getValue());

			Optional<Endpoint> endpoint = endpoints.stream().filter(e -> pathLiteral.getValue().equals(e.getFullPath()))
					.findFirst();

			if (edge.getSource() instanceof PyIs isCheck) {

				entityObtainedFromPathVar = Arrays.stream(isCheck.getSubExpressions()).toList()
						.get(1) instanceof NullLiteral;
				...
				if (entityObtainedFromPathVar) {
					...
					if (!entityPresenceChecked) {
						tool.warnOn(checkByNull.getCFG(), "DELETE endpoint" + pathLiteral.getValue()
								+ " is missing prior entity check-up for its existence before deletion.");
						endpoint.ifPresent(value -> value.addIssue(
								"Endpoint is missing prior entity check-up for its existence before deletion."));
					}
				}

			} else {
				tool.warnOn(edge.getSource(), "DELETE endpoint " + pathLiteral.getValue()
						+ " deletes entity without prior check-up for its existence.");
				endpoint.ifPresent(
						value -> value.addIssue("Endpoint deletes entity without prior check-up for its existence."));
