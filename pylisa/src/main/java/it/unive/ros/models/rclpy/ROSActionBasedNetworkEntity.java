...
abstract class ROSActionBasedNetworkEntity extends ROSNetworkEntity<ROSActionChannel>{
    ...
    public ROSActionBasedNetworkEntity(ROSNetwork network, ROSActionChannel channel, ROSNode node) {
        super(network, channel, node);
    }

    public ROSActionBasedNetworkEntity(ROSNetwork network, ROSActionChannel channel, String nodeID) {
        super(network, channel, nodeID);
    }

    public ROSActionBasedNetworkEntity(String containerID, ROSActionChannel channel, String msgType, HeapExpression expr, Statement publisherStmt, AnalysisState<SimpleAbstractState<PointBasedHeap, ValueEnvironment<ConstantPropagation>, TypeEnvironment<InferredTypes>>> analysisState) {
