...
public class ROSNetwork
		extends
		Network<ROSNode, ROSNetworkEntity<? extends ROSCommunicationChannel>, ROSCommunicationChannel> {

	@Override
	public void addEntityContainer(
			ROSNode container)
			throws Exception {
		...
		if (container.getEnableParameterServices()) {
			...
			addNetworkEntity(
					new ROSServiceServer(this, serviceChannel, container, "rcl_interfaces/srv/DescribeParameters"),
					container.getID());
			serviceChannel = new ROSServiceChannel(container.getURI() + "/get_parameter_types", true);
			addNetworkChannel(serviceChannel);
			addNetworkEntity(
					new ROSServiceServer(this, serviceChannel, container, "rcl_interfaces/srv/GetParameterTypes"),
					container.getID());
			serviceChannel = new ROSServiceChannel(container.getURI() + "/get_parameters", true);
			addNetworkChannel(serviceChannel);
			addNetworkEntity(new ROSServiceServer(this, serviceChannel, container, "rcl_interfaces/srv/GetParameters"),
					container.getID());
			serviceChannel = new ROSServiceChannel(container.getURI() + "/list_parameters", true);
			addNetworkChannel(serviceChannel);
			addNetworkEntity(new ROSServiceServer(this, serviceChannel, container, "rcl_interfaces/srv/ListParameters"),
					container.getID());
			serviceChannel = new ROSServiceChannel(container.getURI() + "/set_parameters", true);
			addNetworkChannel(serviceChannel);
			addNetworkEntity(new ROSServiceServer(this, serviceChannel, container, "rcl_interfaces/srv/SetParameters"),
					container.getID());
			serviceChannel = new ROSServiceChannel(container.getURI() + "/set_parameters_atomically", true);
			addNetworkChannel(serviceChannel);
			addNetworkEntity(
					new ROSServiceServer(this, serviceChannel, container, "rcl_interfaces/srv/SetParametersAtomically"),
					container.getID());
		}
		if (container.getEnableRosout()) {
			...
			addNetworkEntity(new ROSTopicPublisher(container, topic, "rcl_interfaces/msg/ParameterEvent"),
					container.getID());
		}
		...
		addNetworkEntity(new ROSTopicPublisher(container, topic, "rmw_dds_common/msg/ParticipantEntitiesInfo"),
				container.getID());
		addNetworkEntity(new ROSTopicSubscription(container, topic, "rmw_dds_common/msg/ParticipantEntitiesInfo", null),
				container.getID());

		ROSServiceChannel serviceChannel = new ROSServiceChannel(container.getURI() + "/get_type_description", true);
		addNetworkChannel(serviceChannel);
		addNetworkEntity(new ROSServiceServer(this, serviceChannel, container,
				"type_description_interfaces/srv/GetTypeDescription"), container.getID());
	...
	public ROSActionChannel getActionChannel(
			String actionName) {
		for (ROSCommunicationChannel channel : getNetworkChannels()) {
	...
	public void addNetworkEntity(
			ROSNetworkEntity<? extends ROSCommunicationChannel> ne,
			String networkEntityContainerID) {
	...
	public ROSServiceChannel getServiceChannel(
			String serviceName) {
		for (ROSCommunicationChannel channel : getNetworkChannels()) {
		...
		for (ROSServiceChannel s : this.getServices()) {
			if (!s.isSystem()) {
				if (this.getChannelUserContainers(s).isEmpty()) {
					...
					mermaid.append("style ").append(s.getName())
							.append(" fill:lightskyblue,stroke:#333,stroke-width:2px\n");
				} else {
					mermaid.append(s.getName()).append("[/").append(s.getName()).append("/]:::wide\n");
					mermaid.append("style ").append(s.getName())
							.append(" fill:lightskyblue,stroke:#333,stroke-width:2px\n");
	...
	public String toGraphviz(
		...
		for (ROSTopic t : this.getTopics()) {
			if (!t.isSystem()) {
				if (this.getChannelUserContainers(t).isEmpty()) {
					dotGraph.append("\"").append(t.getID()).append("\"")
		...
		for (ROSServiceChannel s : this.getServices()) {
			...
			if (!s.isSystem()) {
				/*
				 * if (this.getChannelUserContainers(s).isEmpty()) {
				 * dotGraph.append("\"").append(s.getName()).append("\"").append
				 * (
				 * "[shape=parallelogram,style=filled,fillcolor=\"lightskyblue\"];"
				 * ); } else {
				 * dotGraph.append("\"").append(s.getName()).append("\"").append
				 * (
				 * "[shape=parallelogram,style=filled,fillcolor=\"lightskyblue\"];"
		...
		for (ROSNode n : getNetworkEntityContainers()) {
			for (ROSTopicPublisher p : n.getPublishers()) {
				if (!p.getChannel().isSystem()) {
					dotGraph.append("\"").append(n.getURI()).append("\"").append(" -> ").append("\"")
							.append(p.getChannel().getID()).append("\"");
				}
			}
			for (ROSTopicSubscription s : n.getSubscribers()) {
				if (!s.getChannel().isSystem()) {
					dotGraph.append("\"").append(s.getChannel().getID()).append("\"").append(" -> ").append("\"")
							.append(n.getURI()).append("\"");
				}
			}
			for (ROSServiceServer ss : n.getServiceServers()) {
				if (!ss.getChannel().isSystem()) {
					for (ROSTopicBasedNetworkEntity t : ss.toTopicEntities()) {
						if (t instanceof ROSTopicPublisher) {
							dotGraph.append("\"").append(n.getURI()).append("\"").append(" -> ").append("\"")
									.append(t.getChannel().getID()).append("\"");
						} else {
							// is a Subscription
							dotGraph.append("\"").append(t.getChannel().getID()).append("\"").append(" -> ")
									.append("\"").append(n.getURI()).append("\"");
						}
					}
				}
			}
	...
	public Set<ROSNode> getChannelUserContainers(
			ROSCommunicationChannel channel) {
