	...
	public void processMessage(
		...
		if (nodeExpr instanceof HeapReference) {
			...
			if (subscriptionStmt instanceof UnresolvedCall) {
				...
				// subCallback.forwardSemanticsAux(container.getLisaState().interproceduralAnalysis,
		...
		System.out.println("[" + this.getContainer().getName() + "::" + this.getChannel().getID() + "] RECEIVED ["
				+ message.getMessage() + "] FROM: [" + message.getNetworkEvent().getInitiator().getContainer().getName()
				+ "]");
	}
