...
public class ROSServiceServer extends ROSServiceBasedNetworkEntity {
	...
	public Set<ROSTopicBasedNetworkEntity> toTopicEntities() {
		...
		topics.add(new ROSTopicPublisher(null, new ROSTopic(getChannel().getName() + "Reply", getChannel().isSystem(), ROSTopicType.SERVICE_RESPONSE, getChannel().isAvoidRosNamespaceConventions()), getType(), null, null, null));
		topics.add(new ROSTopicSubscription(null, new ROSTopic(getChannel().getName() + "Request", getChannel().isSystem(), ROSTopicType.SERVICE_REQUEST, getChannel().isAvoidRosNamespaceConventions()), getType(), null, null, null));
		return topics;
	}
