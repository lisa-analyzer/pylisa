...
public class ROSApplication {
    ...
    public ROSApplication(RosComputationalGraph ROSComputationalGraph, Map<String, Grant> permissionsGrants, String workDir) {
        this.ROSComputationalGraph = ROSComputationalGraph;
        this.workDir = workDir;
    ...
    public void dumpGraph() throws Exception {
        ...
        for(Node n : ROSComputationalGraph.getNodes()) {
            for (Service s : n.getServices()) {
                dotGraph.append("subgraph cluster_" + i +" { style=filled;fillcolor=white;color=blue;penwidth=2;fontcolor=blue;label=\"" + s.getName() + "\";").append("\"" + s.getName() + "Reply\"").append("[shape=box,style=filled,fillcolor=\"tan1\"];").append("\"" + s.getName() + "Request\"").append("[shape=box,style=filled];}");
                i++;
            }
            dotGraph.append("\"" + n.getName() + "\"").append("[style=filled,fillcolor=\"aquamarine\"];");
        }
        for (Topic t : ROSComputationalGraph.getTopics()) {
            if (ROSComputationalGraph.getTopicSubscriptions(t.getName()).isEmpty() || ROSComputationalGraph.getTopicPublishers(t.getName()).isEmpty()) {
    ...
    public void dumpSecureGraph() throws Exception {
        ...
        for(Node n : ROSComputationalGraph.getNodes()) {
            for (Publisher p : n.getPublishers()) {
                ...
                // PermissionsNode permissionsNode = permissionsNodes.get(n.getName())
                /*Node _n = permGraph.getNodeByName(n.getName());
                if (_n != null) {
                    for (Publisher _p : permGraph.getNodeByName(n.getName()).getPublishers()) {
                        if (_p.getTopic().getName().equals(p.getTopic().getName())) {
